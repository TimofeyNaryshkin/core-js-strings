...
function removeLastOccurrences(str, value) {
  if (str.lastIndexOf(value) !== -1) {
    const before = str.slice(0, str.lastIndexOf(value));
    const after = str.slice(str.lastIndexOf(value) + value.length, str.length);
    return before + after;
  }
  return str;
}
...
function isPalindrome(str) {
  ...
  const reversedString = str
    ...
    .join('');
  if (
    reversedString.toLowerCase().replaceAll(' ', '') ===
...
function encodeToRot13(str) {
  let newStr = '';
  for (let i = 0; i < str.length; i += 1) {
    const encodedChar = str.charCodeAt(i);
    if ((encodedChar > 77 && encodedChar < 97) || encodedChar > 109) {
      newStr += String.fromCharCode(encodedChar - 13);
    } else if (encodedChar > 64 && encodedChar < 110) {
      newStr += String.fromCharCode(encodedChar + 13);
    } else {
      newStr += str[i];
    }
